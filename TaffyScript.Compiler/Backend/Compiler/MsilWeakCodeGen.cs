    ...
    public class MsilWeakCodeGen : ISyntaxElementVisitor
    {
        ...
        public void Visit(ArrayAccessNode arrayAccess)
        {
            ...
            if (CanBeArrayAccess(arrayAccess))
            {
                ...
                Type[] argTypes;
                var count = arrayAccess.Arguments.Count - 1;
                if (count > 2)
                {
                ...
                LoadElementAsInt(arrayAccess.Arguments[count]);
        ...
        public void Visit(ImportObjectNode importNode)
        {
            ...
            else
            {
                        ...
                        case PropertyInfo pi:
                            ...
                            if (memberType == typeof(TsObject) || TsTypes.ObjectCasts.ContainsKey(memberType))
                            {
                                    ...
                                    (pi.CanWrite && pi.SetMethod.GetParameters().Length > 1))
                                {
                                    ...
                                    {
                                        foundIndexer = true;
                                        AddMethodToTypeWrapper(pi.GetMethod, weakFlags, type, "get", importNode.Position, source, call, tryd, readOnly, instanceMethods);
                                    }
                                    if(pi.CanWrite && IsMethodSupported(pi.SetMethod))
                                    {
                                        foundIndexer = true;
                                        AddMethodToTypeWrapper(pi.SetMethod, weakFlags, type, "set", importNode.Position, source, call, tryd, readOnly, instanceMethods);
                                    }
                                }
                                else
                                {
                ...
                foreach(var mi in validMethods)
                    AddMethodToTypeWrapper(mi, weakFlags, type, transformName(mi.Name), importNode.Position, source, call, tryd, readOnly, instanceMethods);
