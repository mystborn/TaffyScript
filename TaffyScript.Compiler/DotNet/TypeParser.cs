...
namespace TaffyScript.Compiler.DotNet
{
    ...
    public class DotNetTypeParser
    {
        ...
        private static Regex _typeRegex = new Regex(@"(?<Base>[a-zA-Z_][a-zA-Z0-9_.]*)\s*(\<(?<Generic>(\s*,?\s*[a-zA-Z_][a-zA-Z0-9_]*\s*(\<.+\>)?(\[(\s*,)*\s*\])?)+)\>)?\s*(?<Rank>\[(\s*,)*\s*\])?", RegexOptions.Compiled);
